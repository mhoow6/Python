    ...
    while True:
        try:
            ...
            for conn in group:
                ...
                if recv[1] != conn:  # client 본인이 보낸 메시지는 받을 필요가 없기 때문에 제외
...
def Recv(conn, count, send_queue):
    ...
    while True:
        ...
        send_queue.put([data, conn, count])  # 각각의 클라이언트 메시지, 소켓정보, 쓰레드 번호를 send로 보냄
...
if __name__ == '__main__':
    ...
    server_sock.bind((HOST, PORT)) # 소켓에 수신받을 IP 주소에 PORT를 설정
    server_sock.listen(10) # 소켓 연결, 10개의 동시접속 허용
    count = 0
    group = [] # 연결된 클라이언트의 소켓 정보를 리스트로 묶기 위함
    while True:
        count = count + 1
